...
| | 치안(`sf_*`)이 실제로 쓰이는 곳 | 현재 상태 |
|---|---|---|
| 매출 예측 모델 | **학습 피처에 포함** (`model_metadata.json` → `chosen_step: E_plus_safety`, ablation 에서 MAE 개선) | **값이 NaN** — 모델이 '모름'으로 처리 (예측값 평균 차이 ≈1.9%) |
| 종합진단 `overall_score` (= 기회점수의 원천) | `safety_score` 가중 **5%** (`scoring_weights.yaml`) | NaN 성분은 제외되고 나머지 가중치가 **재정규화** → 실질 미반영 |
| 웹 종합점수 '치안 반영' 토글 | `/api/safety` (범죄 2024·CCTV 2025 **실측**) 가중 5% | **동작 중** — 사용자 옵트인, 모델 파이프라인과 독립 |

즉 **정본은 치안을 학습·점수 양쪽에 쓰도록 설계돼 있으나 현재 서빙 값이 비어 있고**, 화면에서
의미 있게 작동하는 치안은 웹이 별도로 조달한 토글 하나다. 외부 데이터를 파이프라인에 연결하면
(§6) 세 줄이 모두 살아난다 — [docs/OPEN-ITEMS.md](docs/OPEN-ITEMS.md) 참조.
...
- 치안(⑥)은 **자치구 단위 실측**(범죄 2024·CCTV 2025). 정본 모델은 치안을 학습 피처로 쓰도록
  설계돼 있으나 **현재 서빙 값이 NaN 이라 예상 매출·기회점수에는 반영되지 않았고**, 화면에서
  작동하는 것은 종합점수의 **사용자 옵트인 토글(5%)** 뿐이다 — 산식·출처를 화면에 노출한다
- 배후지(⑦)는 실데이터 미보유 구간으로 **예시 데이터**임을 배너로 표시한다
