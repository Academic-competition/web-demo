...
**범죄·CCTV 데이터는 메인 전처리에 들어가지 않는다.** 자치구×연 단위라 상권×업종×분기
표와 병합 키가 달라, 별도 가공(10만명당 발생률·검거율)을 거쳐 자치구 안전점수(0~100)가
된 뒤 두 지점 중 하나로 합류한다 — 정본(Commercial-AI-) 설계는 서빙테이블에 as-of 조인
(점선), 현재 배포는 `/api/safety` 가 화면 계층에서 상권의 소속 구에 매칭(실선).
어느 경로든 **매출 예측 모델의 입력에는 범죄가 들어가지 않으며**, 종합점수의
'치안 반영' 토글(사용자 옵트인, 가중 5%)에만 관여한다.
...
- 치안(⑥)은 **자치구 단위 실측**(범죄 2024·CCTV 2025)이며, 매출 예측·창업기회점수에는
  들어가지 않고 종합점수에만 **사용자가 토글을 켤 때 5%** 가중된다. 산식·출처를 화면에 노출한다
