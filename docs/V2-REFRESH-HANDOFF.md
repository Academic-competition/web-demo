...
> ## ✅ 2026-08-05 — 이 문서의 작업은 완료됐다
>
> 파이프라인 머신에서 8/4~8/5 에 걸쳐 전부 반영·배포됐다 (web-demo `fc781d0`~`6d61943`):
>
> - §2 산출물 재생성 ✅ (추이 5포인트 1,387건 — 기준치 일치 · 매출 하향 검증:
>   편의점 0.25배·치킨 0.37배·한식 0.90배)
> - §3-1 과대추정 문구 ✅ · §3-2 이중보정 제거 ✅ (`correctStoreCounts` → `readStoreCounts`)
> - §3-3 치안 서술 반전 ✅ (README 표·ResultPanel·data-flow.svg·OPEN-ITEMS §0)
> - §3-4 `detail.safety` ✅ — 계획보다 크게: 치안 전체(⑥ 카드 + 안전점수)를
>   **모델 저장소 단일 소스**로 통합 (`export_web_static.py` 의 `load_safety_by_gu()`)
> - 추가: ⑦ 배후지 상주·직장인구도 모델 소스로 교체 (`rebuild_hinterland()`,
>   구판과 1,633건 전수 일치 확인)
> - §4 (v2 신규 필드 활용 — 독립점포 k·업종별 SMAPE·상권유형 뱃지)는 **미착수** —
>   유일하게 남은 항목이다
>
> 이제 이 문서에서 살아있는 부분은 **아래 부록(머신 분업 규칙)** 뿐이다. 나머지는 역사 기록.
...
> 🔄 **2026-08-05 갱신 — 운영 규칙이 단순해졌다.**
>
> - **개발 PC = 웹 코드·문서 전담.** UI/UX·컴포넌트·`lib/` 로직·문서 수정에 제약이 없다.
>   `model-exports/` 가 커밋돼 있어 **clone + pull 만으로 실데이터 데모가 완전 동작**한다
>   (카카오 키만 그 머신 `.env.local` 에 필요). 데이터를 만들 일이 없다
> - **이 머신 = 데이터 전담.** 서빙 테이블이 갱신되거나 `export_web_static.py` 로직이
>   바뀌면 **여기서만** 재생성 → 커밋한다
> - **개발 PC 에서 `export_web_static.py` 를 돌리지 말 것.** 죽지는 않지만 두 가지가 깨진다:
>   1. `sales/population/store.csv` 원본이 2025Q1~ 뿐 → **추이 차트 5→2포인트 퇴화**
>      (8/3 실제로 겪어 커밋 보류했던 그 문제)
>   2. `resident/worker_population.csv` 미보유 (gitignore, 이 머신 로컬 전용) →
>      배후지 상주·직장인구 블록 재생성이 **조용히 건너뛰어진다** (`rebuild_hinterland()`
>      가 None 반환, 기존 gz 유지 — 방어는 돼 있지만 갱신도 안 된다)
>   실수로 돌려서 `model-exports/` diff 가 생겼다면 **커밋하지 말 것** — gzip 이
>   결정적(mtime=0)이라 diff = 데이터 변화인데, 개발 PC 발 diff 는 퇴화다. `git checkout`
>   으로 되돌린다
> - 치안(⑥)만은 예외적으로 개발 PC 에서도 재생성 가능해졌다 — 원본이
>   `Commercial-AI-/data/raw/external/` 에 커밋됐기 때문. 그래도 위 1·2 때문에
>   export 전체 실행은 여전히 금지다
> - 아래 표의 "포털 CSV 변환기로 export 재생성" 우회로는 **폐기** — 추이 분기는 해결해도
>   인구 CSV 부재(2번)는 해결하지 못한다. `convert_portal_csvs.py` 는 역사 기록으로 남긴다

당시 기록 (2026-08-04):

| | 개발 PC (`C:\toy\academic-competition`) | 이 머신 (`C:\_develop\academy`) |
|---|---|---|
| 가능 | 웹 코드·문서, 포털 CSV 기반 도구(치안·배후지·검증), ~~포털 CSV 변환기로 export 재생성(2024 분기 확보 시)~~ → 폐기, 상단 참조 | 파이프라인 전체(수집→학습→서빙→export) |
| 불가 | 모델 학습·서빙 테이블 생성 (.venv 없음), 2024 분기 이전 추이 (원본 미보유), **인구 CSV (gitignore — export 의 배후지 재생성 불가)** | — |
