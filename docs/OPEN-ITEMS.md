...
> 2. ~~v2 신규 필드 활용~~ → **✅ 8/5 웹 코드 완료 (개발 PC)** — 독립점포 카드(④)·상권유형
>    뱃지(헤더)·업종별 SMAPE 신뢰도(③)·**시장 단계(① 경쟁 불릿)** + 계약/normalize/계보.
>    **전부 optional 설계라 지금 배포돼도 화면 변화 없음(대기 상태).** 스크래치 번들로
>    네 UI 렌더 검증 완료.
>    🔔 **파이프라인 머신에 요청: `export_web_static.py` 가 바뀌었으니 재생성→커밋 1회**
>    (그 순간 새 섹션 4종이 자동 등장). 재생성 전까지 배포본은 기존 그대로다.
