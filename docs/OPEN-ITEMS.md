...
## 9. 🟡 모델 데이터 편입 — 트랙2 실행 완료, 트랙1 미착수 (2026-08-03)

> **트랙2(상주·직장인구 + 재학습) 실행 완료.** 수집·병합·재학습까지 돌렸고 상권 유형이
> 5종이 됐지만 **ML 성능 기여는 없었다**. 트랙1(치안·부동산 값 채우기)은 미착수이며,
> ablation 실측상 효과도 미미하다(치안 −0.5%, 부동산 +악화).
> ⚠️ 아래 "이 PC 에서는 파이프라인을 돌릴 수 없다"는 **다른 머신 기준 서술**이다 —
> `C:\_develop\academy` 에서는 정상 동작한다(2026-08-02 확인).
