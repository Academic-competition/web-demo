...
> 최종 갱신: 2026-07-28
>
> **지금 이어서 할 일**: [§6 배후지 데이터 수집](#6--배후지-데이터-미수집--다음-작업-1순위-한-번에-3개가-풀린다) —
> 리포트 ⑦ 목업 제거 + ML 수요 피처 공백(§3) + 상권 유형 축소가 한 번에 풀린다.
...
## 6. 🔴 배후지 데이터 미수집 — **다음 작업 1순위** (한 번에 3개가 풀린다)

`Commercial-AI-/config/data_sources.yaml:62,71` 의 서비스명이 `verified: false` 라 자동 수집
대상에서 빠져 있다 (`VwsmTrdarRepopQq` / `VwsmTrdarWrcPopltnQq`). 이 데이터가 없어서 세 가지가
동시에 막혀 있다:

1. **리포트 ⑦ 배후지 분석이 전량 목업** — `lib/mockExtras.ts` 의 `mockHinterland()`.
   주거인구·직장인구·가구수·아파트 비율·소득분위·임대시세·소비트렌드·집객시설 8개가 가상값이며
   ⚠ 배너로 표시된다. **현재 데모에 남은 유일한 목업 구간이다** (⑥ 치안은 §1 에서 실데이터 전환됨)
2. **ML 수요 측 피처 공백** → §3 의 베이스라인 열세
3. **상권 유형 분류 축소** — `총_상주인구_수`·`총_직장인구_수` 결측으로 주거형·직장형이 배정되지
...
- **배후지 분석(⑦) 목업** — §6 으로 이동(다음 작업 1순위). `lib/mockExtras.ts` 의
  `mockSafety()` 는 §1 전환 후 호출되지 않지만 폴백으로 남겨 뒀다
