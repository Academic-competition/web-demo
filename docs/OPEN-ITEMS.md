...
> 최종 갱신: 2026-08-05
>
> ## 📌 현재 남은 것 — 이것만 보면 된다 (2026-08-05)
>
> **기능적으로 열려 있는 항목은 없다.** 모델 v2 반영·치안/배후지 모델 소스 통합·
> v2 신규 필드 9종 운반까지 전부 배포됐다. 남은 것은 판단·선택 과제뿐이다.
>
> | 우선 | 항목 | 성격 | 비고 |
> |---|---|---|---|
> | ~~1~~ | ~~신규 9종 UI 정리~~ → **✅ 8/5 완료** | — | golmok 재실측(Playwright)으로 **"무겁다"는 전제가 뒤집혔다** — 공식 서비스 리포트는 39배 스크롤로 **우리(7.5배)의 5배**다. 제거는 강점·리스크 1건만 하고, 밀도·임대에는 오히려 **비교 기준을 추가**했다. 근거: [BENCHMARK-golmok.md](BENCHMARK-golmok.md) 상단 재실측 표 |
> | 2 | **롤링 백테스트** (§10) | 발표 자료 | "ML 이 이겼다"의 근거가 시험 1회분. 분기별 성적 그래프가 있으면 반박이 사라진다. 모델 저장소 수정 = 팀원 합의 |
> | 3 | 배후지 이월 3종 수집 편입 (§6) | 재현성 | **지금 할 이유 없음.** 화면 정상·유지비 0. 대회 이후 항목 |
