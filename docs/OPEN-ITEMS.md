...
> | 우선 | 항목 | 성격 | 비고 |
> |---|---|---|---|
> | 1 | **리포트 배경 다크 vs 라이트** | 결정 | 배포된 패널 헤더 토글(☀ 밝게 보기)로 비교 — 아래 🎨 ① |
> | 2 | **밀도 3종 랭킹 재생성 + 머지** | 파이프라인 머신 | 브랜치 `feat/density-rankings` — 아래 🎨 ② |
> | 3 | **롤링 백테스트** (§10) | 발표 자료 | "ML 이 이겼다"의 근거가 시험 1회분. 분기별 성적 그래프가 있으면 반박이 사라진다. 모델 저장소 수정 = 팀원 합의 |
> | 4 | 배후지 이월 3종 수집 편입 (§6) | 재현성 | **지금 할 이유 없음.** 화면 정상·유지비 0. 대회 이후 항목 |
