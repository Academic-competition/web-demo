...
## 2. ✅ 예상 매출 과대추정 — 원인 확인·수정 완료 (2026-08-03, 반영 대기)

> **실험 완료.** 분모를 `유사_업종_점포_수` 로 고치고 재학습했더니 ML 이 나이브 규칙을
> 이겼다(test MAE −3.04%). 단 **검증셋도 함께 넓혀야** 한다(`valid_quarters: 1→2`) —
> 분모만 고치면 여전히 검증셋 과적합 모델(LightGBM)이 선정돼 진다.
> 상세: [MODEL-EXPERIMENT-REPORT.md](MODEL-EXPERIMENT-REPORT.md) §3~§4.
> 변경은 브랜치 `fix/target-denominator-and-model-selection` 에 있고,
> **서빙·웹 산출물 재생성은 아직 안 했다** — 배포본은 여전히 수정 전 데이터다.
