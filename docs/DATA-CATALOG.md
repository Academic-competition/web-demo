...
| ① **강점·리스크 목록** (▲/▼ 문장) | `strengths`·`risks` (진단 규칙 템플릿) | 규칙 | 🕐 |
...
| ⑤ **인구 밀도 3종** (명/km² 타일) | `*_density_km2` (실측 ÷ 영역 면적) | 통계 | 🕐 |
...
| ⑦ **임대 시세** (m²당·공실률·지수, 자치구 평균) | `re_rent_per_m2`·`re_vacancy_rate`·`re_rent_index(+yoy)` — R-ONE, `gu_mean` 조인 명시. "제공하지 않는 항목"의 임대시세는 자동 제외 | 실측 | 🕐 |
