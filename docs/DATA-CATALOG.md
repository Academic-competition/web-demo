...
> 기준: 모델 저장소 main `cbaab3d`(v2.0.0, 2025Q2 스냅샷) · 웹 `6c9051b` · 2026-08-05.
> 상태 표기 — ✅ 화면 사용 중 · 🕐 코드 완료, **export 재생성 대기**(파이프라인 머신 1회 실행 시 등장) ·
> ⬜ 모델은 주는데 웹이 아직 안 씀(활용 후보) · 🔧 내부용(화면 비노출이 정상)
...
| ③ **예측 신뢰도 (±N% 오차)** | 성적표 `test_breakdown.by_industry` SMAPE | 모델 채점 | 🕐 |
...
| 필드 | 내용 | 웹 사용처 | 상태 |
|---|---|---|---|
| `overall_score` | 성분 가중합 종합점수 | 상권 내 업종 기회순위(백분위로 변환) | ✅ |
...
| `상권_유형`·`상권_유형_근거` | 5종 분류(혼합 869·직장 423·주거 163·유동 112·주말여가 3) | 헤더 뱃지 (+by-sangwon) | 🕐 |
| `resident/worker/foot_traffic_density_km2` | 인구 밀도 3종 (명/km²) | 지도 레이어·⑦ 보강 후보 | ⬜ |
...
| 외부-부동산 | `re_rent_per_m2`·`re_vacancy_rate`·`re_rent_index`(+조인 메타) | 학습 피처 + 임대부담 점수 재료 (화면 직접 노출 없음) | 🔧/⬜ |
...
| `test_breakdown.by_commercial_type / by_sales_size` | 상권유형·매출규모별 채점 | 발표 자료·후속 분석 | ⬜ |
...
5. 점수 성분 8종 → 종합점수 클릭 시 성분 분해 뷰 (투명성 확장)
6. `walk_forward` → 발표 슬라이드 (분기별 성적 흐름 그래프)
